...
const fetchDriveImageBase64 = async (folderId, appNo, prefix) => {
  ...
  try {
    ...
    const searchFolderId = subFolderId || folderId;
...
const DriveImage = ({ folderId, appNo, prefix, alt, style, fallbackText }) => {
    ...
    resolveAppSubfolder(folderId, appNo, apiKey)
      ...
      .then(async data => {
        ...
        try {
          ...
          const thumbUrl = match.thumbnailLink ? match.thumbnailLink.replace(/=s\d+$/, '=s600') : null;
  ...
  if (!tried && folderId && appNo) {
    return <div style={{ ...style, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' }}><span style={{ fontSize: '10px', color: '#9ca3af' }}>Loading...</span></div>;
  }
  if (imgSrc) {
    return (
      <img src={imgSrc} alt={alt} referrerPolicy="no-referrer" style={style}
        onError={(e) => {
  ...
  return <span style={{ fontSize: '10px', color: '#9ca3af', textAlign: 'center', padding: '4px', whiteSpace: 'pre-line' }}>{fallbackText || alt}</span>;
};
...
const HallTicket = ({ isSidebarMinimized, onFullscreenChange, onModalStateChange }) => {
  ...
  const generateSinglePDF = async (scholarRef, scholarData, download = true) => {
    try {
      ...
      const lm = 15; const rm = pdfW - 15; const contentW = rm - lm;
      let y = 15;
      const checkNewPage = (n = 10) => { if (y + n > pdfH - 20) { pdf.addPage(); y = 15; } };

      // Logo on left, faculty/dept text on right
      if (logoB64) {
      ...
      pdf.text(`FACULTY OF ${(scholarData.faculty || 'ENGINEERING & TECHNOLOGY').toUpperCase().replace('FACULTY OF', '')}`, rm, y, { align: 'right' }); y += 6;
      pdf.setFontSize(10); pdf.setTextColor(0, 0, 0);
      pdf.text(`DEPARTMENT OF ${(scholarData.department || '').toUpperCase()}`, rm, y, { align: 'right' }); y += 6;
      if (logoB64) y = Math.max(y, 38); // ensure header doesn't overlap logo
      pdf.setDrawColor(180, 0, 0); pdf.setLineWidth(0.5); pdf.line(lm, y, rm, y); y += 8;
      ...
      pdf.setFontSize(13); pdf.setTextColor(180, 0, 0); pdf.setDrawColor(180, 0, 0);
      pdf.rect(pdfW / 2 - 28, y - 6, 56, 10); pdf.text('HALL TICKET', pdfW / 2, y, { align: 'center' });
      pdf.setTextColor(0, 0, 0); y += 12;

      const photoX = rm - 32; const photoY = y; const photoW = 32; const photoH = 40;
      const rows = [["Candidate's Name", scholarData.registered_name || scholarData.name || ''], ['Application Number', appNo || ''], ['Date of Birth', scholarData.date_of_birth || ''], ['Gender', scholarData.gender || ''], ['Mode of Study', scholarData.type || scholarData.program_type || 'Full Time'], ['Institute', scholarData.institution || 'SRMIST'], ['Department', scholarData.department || ''], ['Program', scholarData.program || 'Ph.D']];
      const rowH = 8; const labelW = 55;
      rows.forEach(([label, value], i) => {
        const ry = y + i * rowH;
        pdf.setFillColor(245, 245, 245); pdf.setDrawColor(180, 180, 180);
        pdf.rect(lm, ry, labelW, rowH, 'FD'); pdf.rect(lm + labelW, ry, photoX - lm - labelW - 3, rowH, 'D');
        pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); pdf.setTextColor(0, 0, 0); pdf.text(label, lm + 2, ry + 5.5);
        pdf.setFont('helvetica', 'normal'); pdf.setTextColor(180, 0, 0); pdf.text(String(value).substring(0, 50), lm + labelW + 2, ry + 5.5);
      });
      pdf.setDrawColor(150, 150, 150); pdf.setLineWidth(0.3);
      if (photoB64) { const fmt = photoB64.startsWith('data:image/png') ? 'PNG' : 'JPEG'; pdf.addImage(photoB64, fmt, photoX, photoY, photoW, photoH); pdf.rect(photoX, photoY, photoW, photoH); }
      else { pdf.rect(photoX, photoY, photoW, photoH); pdf.setFontSize(7); pdf.setTextColor(150, 150, 150); pdf.text(`P${appNo}`, photoX + photoW / 2, photoY + photoH / 2, { align: 'center' }); }
      const sigY = photoY + photoH + 3;
      if (sigB64) { const fmt = sigB64.startsWith('data:image/png') ? 'PNG' : 'JPEG'; pdf.addImage(sigB64, fmt, photoX, sigY, photoW, 12); pdf.rect(photoX, sigY, photoW, 12); }
      else { pdf.rect(photoX, sigY, photoW, 12); pdf.setFontSize(7); pdf.setTextColor(150, 150, 150); pdf.text('Signature', photoX + photoW / 2, sigY + 7, { align: 'center' }); }
      y += rows.length * rowH + 10; pdf.setTextColor(0, 0, 0);

      checkNewPage(30);
      pdf.setFontSize(10); pdf.setFont('helvetica', 'bold'); pdf.text('EXAMINATION SCHEDULE', lm, y); y += 5;
      const colW = [40, 70, contentW - 110];
      ['Examination', 'Date & Time', 'Reporting Venue'].forEach((h, i) => { const x = lm + colW.slice(0, i).reduce((a, b) => a + b, 0); pdf.setFillColor(230, 230, 230); pdf.rect(x, y, colW[i], 8, 'FD'); pdf.setFontSize(8); pdf.text(h, x + 2, y + 5.5); }); y += 8;
      ...
      [['Written', '16/05/2026, 10:00 AM \u2013 12:00 Noon'], ['Interview', '16/05/2026, 1:00 PM \u2013 5:00 PM']].forEach(([exam, dt], i) => {
        const rowY = y + i * 10; pdf.setFont('helvetica', 'bold'); pdf.setFontSize(8);
      ...
      checkNewPage(10); pdf.setDrawColor(150, 150, 150); pdf.line(lm, y, rm, y); y += 6;
      pdf.setFontSize(10); pdf.setFont('helvetica', 'bold'); pdf.setTextColor(0, 0, 0); pdf.text('INSTRUCTIONS TO CANDIDATES', lm, y); y += 7;
      const instructions = [
        { text: 'I.  Hall ticket is valid for both Written Examination and Interview.' },
        { heading: 'II. Time Schedule:', items: ['Candidates should report to the examination venue 30 minutes before the commencement of the exam.', 'No candidate will be allowed into the examination hall 30 minutes after commencement.', 'Candidates will not be allowed to leave the hall until the end of the examination.', 'Candidates must bring a valid original Photo ID proof (Aadhaar / Passport / Driving License).', 'Electronic gadgets, smart watches, calculators, and mobile phones are strictly prohibited.', 'Malpractice in any form will lead to immediate disqualification.'] },
        { heading: 'III. Examination Guidelines:', items: ['The exam pattern will be Objective Type (MCQ) containing 100 questions.', 'Use only Blue or Black ballpoint pen to shade the OMR sheet.', 'Duration of the examination is 2 Hours (120 minutes).'] },
        { heading: 'IV. Interview Guidelines:', items: ['Interview will be conducted in offline mode only on the same date.', 'Candidates must present their research proposal (max 5 slides) during the interview.', 'Final evaluation is based on combined performance in Written test (70%) and Interview (30%).'] },
      ];
      pdf.setFontSize(8.5);
      instructions.forEach(({ heading, text, items }) => {
        checkNewPage(12);
        if (heading) {
          pdf.setFont('helvetica', 'bold'); pdf.setTextColor(0, 0, 0); pdf.text(heading, lm, y); y += 6;
          pdf.setFont('helvetica', 'normal');
          items.forEach((item, idx) => { const lines = pdf.splitTextToSize(`${idx + 1}.  ${item}`, contentW - 8); checkNewPage(lines.length * 5 + 2); pdf.text(lines, lm + 6, y); y += lines.length * 5 + 1; });
          y += 3;
        } else {
          pdf.setFont('helvetica', 'normal'); pdf.setTextColor(0, 0, 0);
          const lines = pdf.splitTextToSize(text, contentW); checkNewPage(lines.length * 5 + 2); pdf.text(lines, lm, y); y += lines.length * 5 + 4;
        }
      });
      ...
      if (download) { pdf.save(`HallTicket_${appNo || scholarData.id}.pdf`); showMessage(`Hall Ticket generated for ${scholarData.registered_name}`, 'success'); }
      return pdf;
    } catch (error) { console.error('Error generating PDF:', error); showMessage('Error generating Hall Ticket', 'error'); }
  };
    ...
    for (let i = 0; i < scholarsToGenerate.length; i++) {
      ...
      try {
        ...
        rows.forEach(([label, value]) => {
          ...
          pdf.rect(10, y, 55, 7, 'F'); pdf.text(label, 12, y + 5);
          pdf.setFont('helvetica', 'normal'); pdf.text(String(value).substring(0, 60), 67, y + 5);
          pdf.setDrawColor(180, 180, 180); pdf.rect(10, y, 55, 7); pdf.rect(65, y, W - 85, 7);
          y += 7;
        });
        if (photoB64) { pdf.addImage(photoB64, 'JPEG', W - 45, 58, 32, 40); pdf.setDrawColor(150, 150, 150); pdf.rect(W - 45, 58, 32, 40); }
        else { pdf.setDrawColor(150, 150, 150); pdf.rect(W - 45, 58, 32, 40); pdf.setFontSize(7); pdf.setTextColor(150, 150, 150); pdf.text(`P${appNo}`, W - 29, 79, { align: 'center' }); pdf.setTextColor(0, 0, 0); }
        if (sigB64) { pdf.addImage(sigB64, 'JPEG', W - 45, 102, 32, 12); pdf.setDrawColor(150, 150, 150); pdf.rect(W - 45, 102, 32, 12); }
        else { pdf.setDrawColor(150, 150, 150); pdf.rect(W - 45, 102, 32, 12); pdf.setFontSize(7); pdf.setTextColor(150, 150, 150); pdf.text('Signature', W - 29, 109, { align: 'center' }); pdf.setTextColor(0, 0, 0); }
        y += 6; pdf.setFontSize(10); pdf.setFont('helvetica', 'bold'); pdf.text('EXAMINATION SCHEDULE', 10, y); y += 5;
        pdf.setFontSize(8);
        ['Written|16/05/2026, 10:00 AM – 12:00 Noon', 'Interview|16/05/2026, 1:00 PM – 5:00 PM'].forEach(row => {
          ...
          pdf.setDrawColor(180, 180, 180); pdf.rect(10, y, 38, 7); pdf.rect(48, y, W - 58, 7); y += 7;
        });
      ...
      } catch (err) { console.error(`Error generating for ${scholar.application_no}:`, err); }
    }
    setIsGeneratingPdf(false);
  ...
  const getFolderIdForScholar = (scholar) => extractFolderIdFromUrl(scholar?.certificates);
