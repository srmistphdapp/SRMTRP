...
const HallTicket = ({ isSidebarMinimized, onFullscreenChange, onModalStateChange }) => {
  ...
  const generateSinglePDF = async (scholarRef, scholarData, download = true) => {
    try {
      ...
      const rows = [["Candidate's Name", scholarData.registered_name || ''], ['Application Number', appNo || ''], ['Date of Birth', scholarData.date_of_birth || ''], ['Gender', scholarData.gender || ''], ['Mode of Study', scholarData.type || 'Full Time'], ['Institute', scholarData.institution || 'SRMIST'], ['Department', scholarData.department || ''], ['Program', scholarData.program || 'Ph.D']];
