            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left">
                  <input
                    type="checkbox"
                    checked={selectedScholars.length === filteredScholars.filter(s => !s.status?.toLowerCase().includes('forwarded')).length && filteredScholars.filter(s => !s.status?.toLowerCase().includes('forwarded')).length > 0}
