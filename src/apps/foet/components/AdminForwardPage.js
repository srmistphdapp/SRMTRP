...
import { updateScholar } from '../../../services/scholarService';
...
const AdminForwardPage = ({ onBackToDepartment, activeToggle, onToggleChange }) => {
  ...
  const [allDeptsByInstitution, setAllDeptsByInstitution] = useState({});
  const [transferLoading, setTransferLoading] = useState(false);

  // Load institutions and departments from the departments table on mount
  useEffect(() => {
    const loadTransferOptions = async () => {
      const { data, error } = await supabase
        .from('departments')
        .select('faculty, department_name')
        .order('faculty', { ascending: true })
        .order('department_name', { ascending: true });

      if (error || !data) return;

      // Build unique institutions list and grouped departments map
      const grouped = {};
      data.forEach(row => {
        if (!grouped[row.faculty]) grouped[row.faculty] = [];
        grouped[row.faculty].push(row.department_name);
      });

      setAllInstitutions(Object.keys(grouped).sort());
      setAllDeptsByInstitution(grouped);
    };
    loadTransferOptions();
  }, []);
  ...
  useEffect(() => {
    ...
    if (activeFilters.departmentReview !== 'All Reviews') {
      scholars = scholars.filter(s => {
        const review = (s.dept_review || 'Pending').toLowerCase();
  ...
  // Checkbox Handlers — only scholars with faculty_forward === null are selectable
  const selectableScholars = filteredScholars.filter(s => !s.faculty_forward);

  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedScholarIds(selectableScholars.map(s => s.id));
    } else {
  ...
  const performDownload = (type, count) => {
    try {
      let dataToDownload = filteredScholars;
      if (type === 'selected') {
        dataToDownload = filteredScholars.filter(s => selectedScholarIds.includes(s.id));
      } else if (type === 'specific' && count) {
        dataToDownload = filteredScholars.slice(0, count);
      }
  ...
  // Filter departments by selected institution from departments table
  const handleTransferInstitutionChange = (institutionName) => {
    setTransferInstitution(institutionName);
    setTransferDepartment('');
    setTransferDepartments(allDeptsByInstitution[institutionName] || []);
  };

  // Strip "Faculty of " prefix from institution name before saving
  const stripFacultyPrefix = (name) => {
    return (name || '').replace(/^Faculty of\s+/i, '').trim();
  };

  // Confirm transfer — writes institution, department directly
  const confirmTransfer = async () => {
    if (!transferringScholar || !transferInstitution || !transferDepartment) {
      showMessageBox('Please select both institution and department to transfer', 'error');
      return;
    }
    ...
    try {
      ...
      const { error } = await updateScholar(transferringScholar.id, updateData);

      if (error) {
        console.error('Error transferring scholar:', error);
        showMessageBox(`Failed to transfer scholar: ${error.message || 'Unknown error'}`, 'error');
        return;
      }

      setAdminScholarsData(prev => prev.map(s =>
        s.id === transferringScholar.id ? { ...s, ...updateData } : s
      ));

      const scholarName = transferringScholar.registered_name || transferringScholar.name;
      showMessageBox(
        `✅ ${scholarName} transferred to ${stripFacultyPrefix(transferInstitution)} — ${transferDepartment}!`,
        'success'
      );
      ...
      console.error('Error transferring scholar:', error);
      showMessageBox('Failed to transfer scholar', 'error');
    } finally {
  ...
  const isAllSelected = selectableScholars.length > 0 && selectableScholars.every(s => selectedScholarIds.includes(s.id));
  ...
  return (
    <div className="scholar-management-wrapper admin-forward-page">
                  ...
                  <tr key={scholar.id}>
                    <td className="text-center">
                      <input
                        ...
                        disabled={!!scholar.faculty_forward}
                        style={{ margin: '0', transform: 'scale(1.1)', cursor: scholar.faculty_forward ? 'not-allowed' : 'pointer', opacity: scholar.faculty_forward ? 0.4 : 1 }}
                      />
                    </td>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: '0.875rem', color: '#9ca3af' }}>
                        {searchTerm || activeFilters.type !== 'All Types' || activeFilters.departmentReview !== 'All Reviews' || activeFilters.department !== 'All Departments'
            ...
            <div className="p-6">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4">
                  ...
                  <div className="view-field">
                    ...
                    <span className="view-value" style={{ fontSize: '0.9375rem', color: '#1f2937', fontWeight: '500', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '0.75rem', borderRadius: '16px', display: 'block' }}>{selectedScholar.institution || '-'}</span>
                  </div>
                  <div className="view-field">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4">
                  <div className="view-field">
                    ...
                    <span className="view-value" style={{ fontSize: '0.9375rem', color: '#1f2937', fontWeight: '500', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '0.75rem', borderRadius: '16px', display: 'block' }}>{selectedScholar.status || '-'}</span>
                  </div>
                  <div className="view-field">
                    <label className="view-label" style={{ fontSize: '0.65rem', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Institution:</label>
                    <span className="view-value" style={{ fontSize: '0.9375rem', color: '#1f2937', fontWeight: '500', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '0.75rem', borderRadius: '16px', display: 'block' }}>{selectedScholar.institution || '-'}</span>
                  </div>
                  <div className="view-field">
                    <label className="view-label" style={{ fontSize: '0.65rem', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Department:</label>
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="view-field">
                    <label className="view-label" style={{ fontSize: '0.65rem', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Describe In 300 Words; Your Reasons For Applying To The Proposed Program; Your Study Interests/future Career Plans, And Other Interests That Drives You To Apply To The Program.:</label>
                    <span className="view-value" style={{ fontSize: '0.9375rem', color: '#1f2937', fontWeight: '500', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '0.75rem', borderRadius: '16px', display: 'block', whiteSpace: 'pre-wrap', lineHeight: '1.5' }}>{selectedScholar.reasons_for_applying || '-'}</span>
                  </div>
                  <div className="view-field">
                    <label className="view-label" style={{ fontSize: '0.65rem', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Title And Abstract Of The Master Degree Thesis And Your Research Interest In 500 Words:</label>
                    <span className="view-value" style={{ fontSize: '0.9375rem', color: '#1f2937', fontWeight: '500', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '0.75rem', borderRadius: '16px', display: 'block', whiteSpace: 'pre-wrap', lineHeight: '1.5' }}>{selectedScholar.research_interest || '-'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="examination-modal">
              ...
              <div className="examination-modal-body">
                ...
                <div className="examination-filter-group">
                  ...
                  <select
                    value={tempFilters.departmentReview}
                    onChange={(e) => setTempFilters(prev => ({ ...prev, departmentReview: e.target.value }))}
                    className="examination-filter-select"
                  >
                    {getUniqueDepartmentReviews().map(review => (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 rounded-t-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-green-100 text-sm">Select new institution & department</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-5">
                <p className="text-sm font-semibold text-orange-700 mb-2">📌 Current Assignment</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Institution</p>
                    <p className="text-sm font-medium text-gray-800">{transferringScholar.institution || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Department</p>
                    <p className="text-sm font-medium text-gray-800">{transferringScholar.department || '-'}</p>
                  </div>
                </div>
              </div>

              {/* Transfer To — Institution, Department, Type Dropdowns */}
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-2">
                <p className="text-sm font-semibold text-green-700 mb-3">🔄 Transfer To</p>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Select Institution *</label>
                    <select
                      value={transferInstitution}
                      onChange={(e) => handleTransferInstitutionChange(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent bg-white"
                    >
                  ...
                  <div>
                    ...
                    >
                      ...
                      {transferDepartments.map((dept, idx) => (
            ...
            <div className="bg-gray-50 px-6 py-4 rounded-b-xl flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!transferInstitution || !transferDepartment || transferLoading}
                className={`px-6 py-2 rounded-lg transition-colors font-medium flex items-center gap-2 ${!transferInstitution || !transferDepartment || transferLoading
      ...
      <DownloadSelectionModal 
        ...
        totalRows={filteredScholars.length}
        selectedCount={selectedScholarIds.length}
      />
    </div>
  );
};
